...
# Checkpoint: https://drive.google.com/drive/u/0/folders/1iANYLKp4pscNSA1VirSSSrPnt-2BNfzx
...
# 4. Change the project_root variable to the path of the Crystalyze repo
# 5. Copy the crystalyze_inference.py into the top level of the Crystalyze repo and run it
...
os.environ["PROJECT_ROOT"] = (
    "/home/shawn/Documents/projects/Crystalyze"  # TODO: change this to your project root!!!
)
import time
...
if __name__ == "__main__":
    ...
    data_root_path = f"{os.environ['PROJECT_ROOT']}/inference"
    model_path = f"{os.environ['PROJECT_ROOT']}/model_folder"
    evaluate_file_path = f"{os.environ['PROJECT_ROOT']}/evaluate.py"
    number_of_guesses = 1
    ...
    elements_involved = list(atom_Zs)
    ...
    with initialize_config_dir(model_path):
        cfg = compose(
            config_name="hparams", overrides=[f"data.root_path={data_root_path}"]
        )
        ...
        model.hparams["data"]["datamodule"]["datasets"]["test"][0][
            "num_augmented_data"
        ] = 0
        model.hparams["data"]["datamodule"]["datasets"]["val"][0][
            "num_augmented_data"
        ] = 0
        model.hparams["data"]["datamodule"]["datasets"]["train"][
            "num_augmented_data"
        ] = 0
        ...
        test_dataset = InMemoryCrystDataset(
            ...
            lattice_scale_method=model.hparams["data"]["datamodule"]["datasets"][
                "test"
            ][0]["lattice_scale_method"],
